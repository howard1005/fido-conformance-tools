    ...
    let generateExtensions = (subjectPublicKeyBuffer, authorityPublicKeyBuffer) => {
        ...
        let cRLDistributionPoints = DERSequence([
            ...
            DEROctetStringObj(DERSequence([DERSequence([
                DERTaggedObject('a0', DERTaggedObject('a0', DERTaggedObject('86', ASN1Object(hex.encode(UTF8toBuffer('https://pki.certinfra.fidoalliance.org/safetynet/crl/root-s1.crl'))))))
            ])]))
        ])

        let finalSequence = DERSequence([keyUsageSequence, basicConstraint, subjectKeyIndentifier, authorityKeyIndentifier, cRLDistributionPoints])
...
/* ----- TBS ENDS ----- */
    window.generateSafetyNetRoot = (hashingAlg, authorityPrivateKeyJWT, subjectPublicKeyJWTStruct) => {
