...
describe(`
        ...
        WebAuthn-Platform-MakeCred-Req-3

        Test platform processing PublicKeyCredentialRpEntity
...
/* ----- POSITIVE TESTS ----- */
    it(`P-1
        ...
        Send a valid MakeCredential request with rp.icon set to some HTTPS url, and check that API succeeds

    `, () => {
        let publicKey     = generateGoodWebAuthnMakeCredential();
        publicKey.rp.icon = 'https://static.certinfra.fidoalliance.org/testimages/catimage.png';
...
/* ----- NEGATIVE TESTS ----- */
    ...
    it(`F-1
        ...
        Send MakeCredential request with PublicKeyCredentialRpEntity.icon set to some HTTP url, and check that API fails

    `, () => {
        let publicKey = generateGoodWebAuthnMakeCredential();
        publicKey.rp.icon = 'http://static.certinfra.fidoalliance.org/testimages/catimage.png';
        return expectPromiseToFail(navigator.credentials.create({ publicKey }))
    })

    // it(`F-5

    //     Send MakeCredential request with PublicKeyCredentialRpEntity.icon set to some HTTPS url that is then downgraded to HTTP, and check that API fails

    // `, () => {
    //     let publicKey = generateGoodWebAuthnMakeCredential();
    //     publicKey.rp.icon = 'https://static.certinfra.fidoalliance.org/imagedowngrade/catimage.png';
    //     return expectPromiseToFail(navigator.credentials.create({ publicKey }))
    // })
})
