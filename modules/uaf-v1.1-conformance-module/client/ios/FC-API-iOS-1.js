...
describe(`
        ...
        Test the FIDO iOS API for specification defined behaviour.
...
/* ---------- Positive Tests ---------- */
    it(`P-1
            ...
            Call canOpenURL method in order to determine whether some FIDO Client has been registered on the device.

            The call must return "true" in order to indicate that a FIDO Client is available.

    `, () => {
        return authenticator.canOpenUAFURL()
            .then((result) => {
                assert.isTrue(result, 'No FidoUAFClient1 available')
            })
    })

    it(`P-2
        
        Invoke the FIDO Client providing a valid DISCOVER request (UAFxRequestType and Base64EncodedJSON) and check that 
    ...
    it(`P-4
        ...
        Call function "process" providing a valid DISCOVER request (UAFxRequestType and Base64EncodedJSON) and check that
            (a) the call succeeds and that
    ...
    it(`P-5
        ...
        Call function "process" providing valid a CHECK_POLICY request (UAFxRequestType and Base64EncodedJSON) and check that
            (a) the call succeeds and that
    ...
    it(`P-6
        ...
        Call function "process" providing valid a UAF_OPERATION request (UAF registration request) (UAFxRequestType and Base64EncodedJSON) and check that
            (a) the call succeeds and that
    ...
    it(`P-7
        ...
        Call function "process" providing valid a UAF_OPERATION request (UAF authentication request) (UAFxRequestType and Base64EncodedJSON) and check that
            (a) the call succeeds and that
            (b) the uafResponseListener provided as second parameter is called with a UAF_OPERATION_RESULT parameter indicating success.
