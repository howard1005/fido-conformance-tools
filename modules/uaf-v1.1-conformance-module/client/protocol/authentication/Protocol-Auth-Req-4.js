...
describe(`
        ...
        Test Client correctly processing AppID and Facets in Authentication Request
...
/* ----- POSITIVE TESTS ----- */
    describe(`P-1
        ...
        Send three AuthenticationRequest UAF messages for the given metadata statement, with "header.appID" field set to "null", "undefined" and "empty" DOMString correspondingly, wait for the responses, and check that each request succeeds  
    ...
    it(`P-2
        ...
        If testing DOM JS API: Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" field set to the current facet, wait for the response, and check that request succeeds

    `)

    it(`P-3

        If testing DOM JS API: Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" field set to effective "https" URI that has the same effective domain as facetID(facet: https://example.com/login.html and appID: https://example.com/loginIsVerySecure), wait for the response, and check that request succeeds

    `)

    it(`P-4

        Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" set to the "https" URI that leads to the trusted facets list, where current facetID is a member, wait for the response, and check that request succeeds
    ...
    it(`P-5
        ...
        If testing DOM JS API: Send AuthenticationRequest UAF message for the given metadata statement, with facetID set to the sub-domain of the appID(facetID:secure.example.com, appID:example.com), wait for the response and check that request succeeds

    `)

    it(`P-6

        Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" set to the "https" URI that leads to the redirect(HTTP 3XX) to the trusted facets list, that has "FIDO-AppID-Redirect-Authorized" header set to "true", and current facetID is a member of the list, wait for the response and check that request succeeds
...
/* ----- NEGATIVE TESTS ----- */
    it(`F-1
        ...
        Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" set to the "https" URI that leads to the trusted facets list, where current facetID is NOT a member, wait for the response, and check that API returns UNTRUSTED_FACET_ID(0x07) error
    ...
    it(`F-2
        ...
        Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" set to the "https" URI that leads to the trusted facets list, where "trustedFacets" key is missing, wait for the response, and check that API returns UNTRUSTED_FACET_ID(0x07) error
    ...
    it(`F-3
        ...
        Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" set to the "https" URI that leads to the trusted facets list, where "trustedFacets" key is NOT of type SEQUENCE, wait for the response, and check that API returns UNTRUSTED_FACET_ID(0x07) error
    ...
    it(`F-4
        ...
        Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" set to the "https" URI that leads to the trusted facets list, where "trustedFacets" key is an empty SEQUENCE, wait for the response, and check that API returns UNTRUSTED_FACET_ID(0x07) error
    ...
    it(`F-9
        ...
        Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" set to the "https" URI that leads to the trusted facets list, where trustedFacets SEQUENCE contains TrustedFacet with missing "ids" key, wait for the response, and check that API returns UNTRUSTED_FACET_ID(0x07) error
    ...
    it(`F-14
        ...
        If testing DOM JS API: Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" field set to the current facet, that is "http" URI, wait for the response, and check that API returns INSECURE_TRANSPORT(0x02) error

    `)

    it(`F-15

        If testing DOM JS API: Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" field set to the current facet, that is "https" URI, and contains insecure mixed content, wait for the response, and check that API returns INSECURE_TRANSPORT(0x02) error

    `)

    it(`F-16

        If testing DOM JS API: Send AuthenticationRequest UAF message for the given metadata statement, with "header.appID" set to the "https" URI that leads to the trusted facets list, and facetID is a sub-domain of the appID eTLD(effective top-level domain) and is NOT a member(e.g.: appID:https://example.com/facets.json and facetID: https://rogue.example.com) of trusted facet list, wait for the response, and check that API returns UNTRUSTED_FACET_ID(0x07) error
