...
describe(`
    ...
    let registrationAssertions = undefined;
    before(function() {
        this.timeout(30000);
        return getTestStaticJSON('Protocol-Reg-Req-P')
            ...
            .then((response) => {
                registrationAssertions = tryDecodeJSON(response.uafProtocolMessage);
            })
            .catch((error) => {
...
/* ---------- Positive Tests ---------- */
    it(`P-1
        ...
        Check that RegistrationResponse SEQUENCE does NOT contain two responses with the same protocol version. 

    `, () => {
        let foundVersion = false;

        for (let message of registrationAssertions) {
