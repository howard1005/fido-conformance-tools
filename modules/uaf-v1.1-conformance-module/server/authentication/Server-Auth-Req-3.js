...
/* ---------- Positive Tests ---------- */
    ...
    `, () => {
        assert.isObject(authenticationRequest.header.upv, 'UPV MUST be of type Dictionary');
        assert.strictEqual(authenticationRequest.header.upv.major, 1, 'UPV.major MUST be set to 1!');
        assert.strictEqual(authenticationRequest.header.upv.minor, 1, 'UPV.minor MUST be set to 1!');
    })

    it(`P-2

        Operation header must contain "op" field, of type DOMString, and: 
            (a) op must equal to "Auth" member of the Operation enum

    `, () => {
        assert.isString(authenticationRequest.header.op, 'UPV MUST be of type Dictionary');
        assert.strictEqual(authenticationRequest.header.op, 'Auth', 'Operation MUST be set to "Auth"');
    })
    ...
    `, () => {
        if (authenticationRequest.header.appID) {
            assert.isString(authenticationRequest.header.appID, 'appID MUST be of type DOMString!');
            assert.isAtMost(authenticationRequest.header.appID.length, 512, 'appID can be max of 512 characters long!');

            assert.isTrue(authenticationRequest.header.appID.startsWith('https://'), 'appID MUST be a HTTPS URL!');

            if(breakURL(authenticationRequest.header.appID).path) {
                return fetch(authenticationRequest.header.appID)
                .then((response) => {
                    if(authenticationRequest.header.appID !== response.url) {
                        console.log(`Detected redirection from "${authenticationRequest.header.appID}" to "${response.url}"`);
                        return navigator.fido.uafv11.getFIDORedirectHeader(authenticationRequest.header.appID)
                        .then((result) => {
    ...
    `, () => {
        assert.isString(authenticationRequest.header.serverData, 'serverData MUST be of type DOMString!');
        assert.isAtLeast(authenticationRequest.header.serverData.length, 1, 'serverData MUST be at least 1 character long!');
        assert.isAtMost(authenticationRequest.header.serverData.length, 1536, 'serverData can be max of 1536 characters long!');
    })
    ...
    `, () => {
        if(authenticationRequest.header.exts) {
            assert.isArray(authenticationRequest.header.exts, 'exts MUST be of type SEQUENCE!');
