...
/* ---------- Positive Tests ---------- */
    ...
    `, () => {
        assert.isObject(registrationRequest.header.upv, 'UPV MUST be of type Dictionary');
        assert.strictEqual(registrationRequest.header.upv.major, 1, 'UPV.major MUST be set to 1!');
        assert.strictEqual(registrationRequest.header.upv.minor, 1, 'UPV.minor MUST be set to 1!');
    })

    it(`P-2

        Operation header must contain "op" field, of type DOMString, and: 
            (a) op must equal to "Reg" member of the Operation enum

    `, () => {
        assert.isString(registrationRequest.header.op, 'UPV MUST be of type Dictionary');
        assert.strictEqual(registrationRequest.header.op, 'Reg', 'Operation MUST be set to "Reg"');
    })
    ...
    `, () => {
        if (registrationRequest.header.appID) {
            assert.isString(registrationRequest.header.appID, 'appID MUST be of type DOMString!');
            assert.isAtMost(registrationRequest.header.appID.length, 512, 'appID can be max of 512 characters long!');

            assert.isTrue(registrationRequest.header.appID.startsWith('https://'), 'appID MUST be a HTTPS URL!');

            if(breakURL(registrationRequest.header.appID).path) {
                fetch(registrationRequest.header.appID)
                    .then((response) => {
                        if(registrationRequest.header.appID !== response.url) {
                            console.log(`Detected redirection from "${registrationRequest.header.appID}" to "${response.url}"`);
                            return navigator.fido.uafv11.getFIDORedirectHeader(registrationRequest.header.appID)
                                .then((result) => {
    ...
    `, () => {
        assert.isString(registrationRequest.header.serverData, 'serverData MUST be of type DOMString!');
        assert.isAtLeast(registrationRequest.header.serverData.length, 1, 'serverData MUST be at least 1 character long!');
        assert.isAtMost(registrationRequest.header.serverData.length, 1536, 'serverData can be max of 1536 characters long!');
    })
    ...
    `, () => {
        if(registrationRequest.header.exts) {
            assert.isArray(registrationRequest.header.exts, 'exts MUST be of type SEQUENCE!');
